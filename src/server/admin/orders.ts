    ...
    include: {
      ...
      payments: {
        ...
        include: {
          ...
          providerActions: { orderBy: { createdAt: "desc" }, take: 20 },
        },
      },
  ...
  return {
    ...
    payments: order.payments.map((payment) => {
      ...
      return {
        ...
        authorizedAmount: payment.authorizedAmount ? moneyToNumber(payment.authorizedAmount) : null,
        capturedAmount: payment.capturedAmount ? moneyToNumber(payment.capturedAmount) : null,
        parentProviderOrderId: payment.parentProviderOrderId,
        savedCardType: payment.savedCardType,
        paymentOption: payment.paymentOption,
        loanMonth: payment.loanMonth,
        loanDiscountCode: payment.loanDiscountCode,
        splitStatus: payment.splitStatus,
        canCapture: payment.status === "authorized" && payment.captureMode === "manual",
        canRefund,
        ...
        providerActions: payment.providerActions.map((action) => ({
          id: action.id,
          type: action.type,
          status: action.status,
          providerActionId: action.providerActionId,
          normalizedStatus: action.normalizedStatus,
