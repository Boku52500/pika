...
Products (including **Cloudflare R2 image uploads**, generic variants, specification groups), categories, brands, orders (fulfillment status plus BOG payment attempts — payment status is provider-controlled; see `docs/payments-bog.md`), and promotions.
